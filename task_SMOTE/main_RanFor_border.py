...
from smote_model_borderline import SMOTEModel
...
while fraud_count <= 4000:
    ...
    model_RanFor = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=1)
    model_RanFor.fit(X_loop_train, y_loop_train)
    ...
    filename = f"task_SMOTE/models/RanFor_borderline/model_fraud_{fraud_count}.joblib"
    joblib.dump(model_RanFor, filename)
...
plt.savefig("task_SMOTE/plots_and_figures/RanFor_Borderline.png")
plt.show()
